...
const OwnerAuth = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatusMessage(authMode === 'login' ? 'Logging you in...' : 'Creating your owner account...');
    setErrorMessage('');

    try {
      if (authMode === 'login') {
        const { data } = await AuthApi.loginOwner(loginForm);
        ...
        <Container className="py-10">
          <div className="mx-auto max-w-md glass-panel rounded-3xl p-6 sm:p-7">
            <h1 className="text-xl font-semibold tracking-tight text-slate-50 sm:text-2xl">
              {authMode === 'login' ? 'Owner Login' : 'Owner Registration'}
            </h1>
            <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
              {authMode === 'signup' && (
                <>
                  <div className="space-y-1.5">
                    <label htmlFor="owner-name" className="text-xs font-medium text-slate-200">
                      Full Name
                    </label>
                    <input
                      id="owner-name"
                  ...
                  <div className="space-y-1.5">
                    <label htmlFor="owner-mobile" className="text-xs font-medium text-slate-200">
                      Mobile Number
                    </label>
                    <input
                      id="owner-mobile"
              ...
              <div className="space-y-1.5">
                <label htmlFor="owner-email" className="text-xs font-medium text-slate-200">
                  Email
                </label>
                <input
                  id="owner-email"
              ...
              <div className="space-y-1.5">
                <label htmlFor="owner-password" className="text-xs font-medium text-slate-200">
                  Password
                </label>
                <input
                  id="owner-password"
            ...
            <p className="mt-4 text-[11px] text-slate-500">
              Verified owners will get priority placement in AI matches once documents are approved
              by admins.
            </p>

            <div className="mt-4 text-center text-[11px] text-slate-500">
              Looking for a home instead?{' '}
              <Link to="/tenant" className="font-medium text-fuchsia-300 hover:text-fuchsia-200">
                Continue as tenant
              </Link>
            </div>
          </div>
